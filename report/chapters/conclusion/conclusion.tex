\chapter{Conclusion}
	...
	We were able to study the observables energy $E$ (\cref{sec:res:energy:observable}), total absolute magnetization $\lvert M^2 \rvert$ (\cref{sec:res:magnet:observable}), specific heat $C_V$ (\cref{sec:res:cv:observable}) and magnetic susceptibility $\chi$ (\cref{sec:res:xs}). Additionally, critical slowing-down effects were observed in~\Cref{sec:res:energy:observable} and, by implementing the Wolff single cluster algorithm, were significantly reduced
	
	In~\Cref{sec:res:temperature}, we used the shifted temperature $T^*$~\citep{shifted} to estimate the critical temperature $T_C$ for the Metropolis algorithm
	\begin{equation}
